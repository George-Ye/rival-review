...
- `.rival-review/review-schema.json` = Codex output schema (auto-generated by `init`)
...
- Elapsed time (15s heartbeat)
- Live progress: files being read, commands being run (with `--verbose`)
...
# Override model, reasoning effort, or timeout
python3 rival_review.py review --model o3 --reasoning-effort high --timeout 600

# Show file reads and commands during review
python3 rival_review.py review --verbose
...
| after_approval | present_plan_only | contract.json | set in contract.json |
...
6. **Workspace guard**: runner checks `git status` before/after each review round.
...
14. **Each round archived** with: draft, prompt, raw JSONL, review, transport
    ...
    Failed attempts are preserved as `round-NNN-attempt-M/` before retry.
